...
class PodcastManager:
    ...
    def fetch_episodes(self, rss_url: str, count: int = 1) -> List[Dict]:
        """
        ...
        try:
            ...
            # Download RSS feed
            response = requests.get(rss_url, timeout=self.config.rss_timeout, headers={"User-Agent": "PodcastPlayer/1.0"})
    ...
    def download_episode(self, episode: Dict, podcast_id: str) -> Optional[str]:
        ...
        try:
            ...
            response = requests.get(episode["url"], stream=True, timeout=self.config.download_timeout, headers={"User-Agent": "PodcastPlayer/1.0"})
            response.raise_for_status()
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        ...
                        if total_size > 0:
                            ...
                            # Only log on new 10% increment
                            if percent // 10 > last_percent_logged:
                                last_percent_logged = percent // 10
                                logger.debug(f"Download progress: {int(last_percent_logged*10)}%")
    ...
    def get_storage_info(self) -> Dict:
        ...
        return {"total_size_mb": total_size / 1024 / 1024, "episode_count": episode_count, "episodes_dir": str(self.episodes_dir)}
