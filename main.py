...
class PodcastPlayer:
    def __init__(self, config_file: str = "config.json"):
        ...
        self.podcast_mgr = PodcastManager(
            self.config["episodes_dir"], self.config["max_episodes_per_podcast"]
        )
    ...
    def check_for_new_episodes(self):
        ...
        for idx, podcast_config in enumerate(self.config["podcasts"]):
            ...
            episodes = self.podcast_mgr.fetch_latest_episodes(
                podcast_config["rss_url"], count=1  # Only check newest episode
            )
            ...
            podcast_state["episodes"] = new_episodes[
            ...
            while True:
                ...
                if GPIO_AVAILABLE:
                    ...
                    if switch_state != last_switch_state:
                        debug_log(
                            f"Switch state changed: {last_switch_state} → {switch_state}"
                        )
